...
| `tier_model`               | The tier pricing model [^tabletext]. Possible values are `Simple Tier`, `Graduated Tier`, `Linear Tier`, or `Block (Step) Tier`. For this example it's `Linear Tier`. For more information, see [How you're charged](/docs/enterprise-management?topic=enterprise-management-charges#charges-services). |
